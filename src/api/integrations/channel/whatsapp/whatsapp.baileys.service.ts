...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private async createClient(number?: string): Promise<WASocket> {
    ...
    const socketConfig: UserFacingSocketConfig = {
      ...
      patchMessageBeforeSending(message) {
        // Não usar JSON.parse(JSON.stringify): isso remove o prototype de Long do protobuf
        // e quebra a codificação no Baileys (TypeError: this.isZero is not a function).
        const normalizeListType = (list?: proto.Message.IListMessage | null) => {
          if (list?.listType === proto.Message.ListMessage.ListType.PRODUCT_LIST) {
            list.listType = proto.Message.ListMessage.ListType.SINGLE_SELECT;
          }
        };

        normalizeListType(message.deviceSentMessage?.message?.listMessage);
        normalizeListType(message.listMessage);
        // zqbaileys mdPatch envolve listMessage em documentWithCaptionMessage — o listType fica no interior.
        normalizeListType(message.documentWithCaptionMessage?.message?.listMessage);
  ...
  private async sendMessage(
    ...
    // interactiveMessage: relay simples não injeta nós biz/native_flow/bot → apps não renderizam botões (PENDING).
    // zqbaileys_helper reproduz o binário que o WhatsApp Web envia (ver README do pacote).
    if (message['interactiveMessage']) {
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const { sendInteractiveMessage } = require('zqbaileys_helper');
        const m = await sendInteractiveMessage(this.client, sender, message, {
        ...
        this.logger.error(['sendInteractiveMessage (zqbaileys_helper)', err?.message, err?.stack]);
        throw new BadRequestException(err?.message || String(err));
      }
    }

    // listMessage: mesmo problema dos botões — relay/sendMessage simples não injeta nós biz/interactive.
    if (message['listMessage']) {
      try {
        const lm = message['listMessage'];
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const { sendListMessage } = require('zqbaileys_helper');
        let listType = proto.Message.ListMessage.ListType.SINGLE_SELECT;
        if (lm.listType != null) {
          const n = Long.isLong(lm.listType) ? lm.listType.toNumber() : Number(lm.listType);
          if (!Number.isNaN(n) && n === proto.Message.ListMessage.ListType.PRODUCT_LIST) {
            listType = proto.Message.ListMessage.ListType.PRODUCT_LIST;
          } else if (!Number.isNaN(n) && n === proto.Message.ListMessage.ListType.SINGLE_SELECT) {
            listType = proto.Message.ListMessage.ListType.SINGLE_SELECT;
          }
        }
        const m = await sendListMessage(
          this.client,
          sender,
          {
            text:
              typeof lm.description === 'string'
                ? lm.description
                : lm.description != null
                  ? String(lm.description)
                  : '',
            footer:
              typeof lm.footerText === 'string' ? lm.footerText : lm.footerText != null ? String(lm.footerText) : '',
            title: typeof lm.title === 'string' ? lm.title : lm.title != null ? String(lm.title) : '',
            buttonText:
              typeof lm.buttonText === 'string' ? lm.buttonText : lm.buttonText != null ? String(lm.buttonText) : '',
            sections: Array.isArray(lm.sections) ? lm.sections : [],
            listType,
          },
          {
            quoted: option?.quoted,
        ...
        this.logger.error(['sendListMessage (zqbaileys_helper)', err?.message, err?.stack]);
        throw new BadRequestException(err?.message || String(err));
      }
    }
  ...
  /**
   * zqbaileys_helper (mdPatch) envolve listMessage / interactive em documentWithCaptionMessage.
   * O conteúdo real fica em documentWithCaptionMessage.message — não assumir documentMessage.
   */
  private hoistDocumentWithCaptionPayload(msg: any): void {
    if (!msg || typeof msg !== 'object') {
      return;
    }
    const wrap = msg.documentWithCaptionMessage;
    if (wrap?.message && typeof wrap.message === 'object') {
      const inner = wrap.message;
      delete msg.documentWithCaptionMessage;
      Object.assign(msg, inner);
    }
  }

  private prepareMessage(message: proto.IWebMessageInfo): any {
    const msgPayload = this.deserializeMessageBuffers({ ...message.message });
    this.hoistDocumentWithCaptionPayload(msgPayload);

    const contentType = getContentType(msgPayload);
    const contentMsg = contentType ? (msgPayload as any)[contentType] : undefined;
    ...
    if (quotedMessage) {
      this.hoistDocumentWithCaptionPayload(quotedMessage);
